...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        mid = self.raw_data[i]["mid"]
        solution = self.raw_data[i]["solution"]
        conversation = self.raw_data[i]["conversation"]
        problem_type = self.raw_data[i]["problem_type"]
        output_type = "text" if task_type == "understanding" else "both"
        # 
        # print(len(conversation))
        # conversation = list(filter(lambda item: item["role"] != "assistant", conversation))
        # print(len(conversation))

        tokenized_conversation = self.tokenize_conversation(conversation, output_type=output_type, add_assistant_start_msg=False)
        # print("dataset info =========>")
        # print("audio_token_ids ",len(tokenized_conversation.audio_token_ids))
        # print("text_token_ids ",len(tokenized_conversation.text_token_ids))
        # print("is_continuous_mask ",len(tokenized_conversation.is_continuous_mask))
        # print("continuous_feature ",len(tokenized_conversation.continuous_feature), len(tokenized_conversation.continuous_feature[0]))
        ...
        # print("============>audio_input_ids ", audio_input_ids.size())
        # print("============>text_input_ids ", text_input_ids.size())
        # print("============>is_continuous_mask ", is_continuous_mask.size(), is_continuous_mask)
        # print("============>ds audio_features ", len(audio_features), len(audio_features[0]), type(audio_features[0]))
        # list 1 960000 ndarray 
        
        # HQ grpo: 修改返回的数据字典 需要包含 目标答案
        ret = dict(
