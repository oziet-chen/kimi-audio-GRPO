...
# JSON_PATH  = "/root/autodl-tmp/dataset/start/material/val60_km.jsonl"
# JSON_PATH  = "/root/autodl-tmp/dataset/start/material/val60_km_val_grponothink.jsonl"
JSON_PATH  = "/root/autodl-tmp/dataset/start/material/val60_km_val_grponothink_semantic_codes.jsonl"
...
# OUT_PATH   = "/root/autodl-tmp/dataset/output/infer-test-val60_km.jsonl"
# OUT_PATH   = "/root/autodl-tmp/dataset/output/infer-test_val60_km_val_grponothink-generate.jsonl"
OUT_PATH   = "/root/autodl-tmp/dataset/output/infer-test_val60_km_val_grponothink_semantic_codes1.jsonl"
...
prompt_manager = KimiAPromptManager(model_path=MODEL_PATH, kimia_token_offset=model.config.kimia_token_offset, kimia_text_audiodelaytokens=model.config.kimia_mimo_audiodelaytokens)
model.config_prompt_manager(prompt_manager, model.config)
# 载入whisper 合并权重 设置eval模式！
model.whisper_model = model.whisper_model.to(torch.cuda.current_device())
model.whisper_model = model.whisper_model.bfloat16()
model.whisper_model.eval()
# or
# model.whisper_model = prompt_manager.whisper_model # 推理用?


# model.whisper_model = WhisperEncoder(os.path.join(MODEL_PATH, "whisper-large-v3"), mel_batch_size=20, unfreeze_online_whisper_model=True)

eval_dataset = dataset_cls(samples, whisper_model=model.whisper_model, text_tokenizer=prompt_manager.text_tokenizer, max_len=8192, kimia_token_offset=model.config.kimia_token_offset)
...
with open(OUT_PATH, "a", encoding="utf-8") as fout:
    for idx, ex in enumerate(samples):
        ...
        if not ex["mid"] in existing_ids: # HQ modify
            messages = list(filter(lambda item: item["role"] != "assistant", ex["prompt"]))
            # with unwrap_model_for_generation(model, Trainer(model).accelerator) as unwrapped_model:
            # _, text = model.generate(messages, **sampling_params, output_type="text")
            input_copy = copy.deepcopy(ex)
            del input_copy["mid"]
            del input_copy["solution"]
            del input_copy["problem_type"]
            del input_copy["prompt"]
            prompt_inputs = input_copy
            _, text, _, _ = model.generate_with_tokens(**prompt_inputs, **sampling_params, output_type="text", max_new_tokens = 256)
        ...
        # --- 评估 ---
        pred = reply
        gt   = list(filter(lambda item: item["role"] == "assistant", ex["prompt"] ))[0]["content"]
        # correct += (pred == gt)
